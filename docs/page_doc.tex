...
在页式虚拟存储计算机中，内存被看作是磁盘的缓存，操作系统通过将常用的虚拟内存页放入物理内存页帧中，将不常用虚拟内存页放入磁盘中来提供更大、更自动化的虚拟存储。页面置换算法决定当页面不足时，需要将哪个页面换出到外存中。尽管很多算法在实践中被证明是有效的，相关的理论分析却相对不足。我们整理了一些基于竞争性分析的工作。竞争性分析通过假设一个“敌人”，说明算法开销与最优算法开销的比的上界，从而增加我们对某种算法的信心。利用竞争性分析，我们可以确定算法在最坏情况下的开销，从而启发我们用其他手段避免算法陷入最坏情景。

第二节中我们讨论常见确定性算法和随机算法的竞争性分析结果，给出确定性算法和随机算法竞争比的下界，讨论常用算法失效的场景；第三节中我们讨论竞争性分析的不足和弥补，通过访存图模型给出更好的下界。
...
使用类似之前的方法，我们定义随机算法R是$c$-Competitive的，如果：
...
\begin{proof}
  考虑访存序列的分割$b_1, b_2, \dots, b_t$，对于一个段$b_i$中某一次访问的页面，如果它在$b_{i-1}$中没有被引用，且这是这个页面第一次在$b_i$中被访问，我们称它是\textbf{新的}；如果它在$b_{i-1}$中被引用过，且这是这个页面第一次在$b_i$中被访问，我们称它是\textbf{旧的}。不妨设$b_i$中对新页面的引用有$l_i$次，对旧页面的引用有$k-l_i$次。

  我们首先证明，最优算法在$b_i$执行的开销至少是$\sum l_i/2$。假设第$b_i$中对新页面的引用发生了$p_i$次缺页，那么在$b_{i-1}$中另外的$l_i-p_i$个新页面都始终在内存中没有被换出，因而在$b_{i-1}$中可用的页面只有$k-(l_i-p_i)$个，至少发生了$l_i-p_i$次缺页。考虑将这两者相加得到：
...
我们调研了基于竞争性分析的对于页面置换策略的分析。竞争性分析给出了一系列下界和上界，基本刻画了各种置换算法的优劣，但由于假设了一个过强的“敌人”，导致分析得到的界与实践经验相去甚远。\cite{onlinealgo}中给出了一些竞争性分析的扩展，但由于时间原因，我们未能对其进行详细的调研。我们仍然关心的问题包括：

\begin{enumerate}
\item 竞争性分析的访存图模型如何被推广到有向图上。这是一个重要的问题，因为程序访存总是依赖于其控制流图的，而控制流图是有向图。如果能够给出有效的算法或分析手段，将给程序访存行为分析带来重要的工具。
\item 能否通过引入随机性“减弱”竞争性分析中假设的敌人？竞争性分析总是给出最坏情况下的分析，而我们也希望拥有某种意义下的“平均情况”分析，这种情形应当和真实情况更加类似。
\item 能否将竞争性分析和工作集模型结合起来，更好的贴合真实程序的访存行为特征？工作集是解释程序访存较好的模型，利用工作集模型表示程序访存并做竞争性分析可能是解决现有困难的有效方法。
\end{enumerate}
...
\bibitem{wikicon} \url{https://en.wikipedia.org/wiki/Page_replacement_algorithm}
